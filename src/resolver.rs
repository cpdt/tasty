...
fn resolve_segment(
    f: &mut dyn fmt::Write,
    segment: &Segment,
    context: &HashMap<Cow<str>, Cow<str>>,
) -> Result<()> {
    match segment {
        Segment::Text(text) => err(f.write_str(text)),
        ...
        Segment::Loop { count, contents } => {
            ...
            let count_int = trimmed_count
                .parse()
                .map_err(|_| Error::ShouldBeInteger(trimmed_count.to_string()))?;

            for index in 0..count_int {
                let derived_context =
                    context.update(Cow::Borrowed("LOOP_INDEX"), Cow::Owned(index.to_string()));
